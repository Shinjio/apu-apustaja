...
def check_perms(ctx, required_level: int):
    ...
    Required_level explicity passed to check_perms in command (currently manually).
    ...
    if msg.author.roles:
        for role in msg.author.roles:
            ...
            if role.id in bot.access_roles[msg.guild.id] and bot.access_roles[msg.guild.id][role.id] >= required_level:
                return True
